...
def digits(x):
    ...
    Returns: A list of digits we want, in the order of 'x'
    """
    digs = []
    while x != 0:
        div, mod = divmod(x, 10)
...
def is_palindrome(x):
    ...
    """
    digs = digits(x)
    for f,r in zip(digs, reversed(digs)):       # Zip takes an iterable and returns an iterator object which is a tuple containing pairs of (digs,reversed(digs))
        if f != r:
